...
function matches(item: CatalogItem, mandate: Mandate, intent: StructuredIntent) {
  ...
  if (q) {
    ...
    const tokens = q.split(/[^a-z0-9]+/).filter((w) => w.length > 2 && !["under", "please", "buy", "need", "some", "the"].includes(w));
...
export function planCart(
  ...
  const candidates = CATALOG.filter((i) => matches(i, mandate, intent)).sort(
